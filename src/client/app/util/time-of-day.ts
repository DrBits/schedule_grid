...
export class TimeOfDay {
    ...
    private setHrMin(hour: number, minutes: number) {
        if (hour < 0 || hour > 23) throw new Error("TimeOfDay: hour must be >= 0 and <= 23")
        if (minutes < 0 || minutes > 59) throw new Error("TimeOfDay: minutes must be >= 0 and <= 59")
        this.hour = hour
    ...
    constructor (p: string | [number, number]) {
        if (typeof p === 'string') {
            ...
            } else throw new Error("TimeOfDay: invalid format, must be xx:yy")
        } else {
